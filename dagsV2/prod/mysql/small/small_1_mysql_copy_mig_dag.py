...
# ==========================================================
# SMALL-1: MySQL -> MySQL 동일 구조 단순 적재 (테이블 그룹 1)
# ==========================================================
...
def _mysql_copy_table(table: str, **context: Any) -> None:
    ...
    try:
        ...
        print(f"[SMALL-1][{table}] {query}", flush=True)
        ...
        while True:
            ...
            tgt_conn.commit()
            total += len(rows)
            print(f" >>> [SMALL-1][{table}] {total} rows...", flush=True)
        print(f"[SMALL-1][{table}] finished total={total}", flush=True)
    except Exception as e:
        print(f"!!! [SMALL-1][{table}] failed: {e}", flush=True)
        raise
    finally:
        if src_conn:
            try:
