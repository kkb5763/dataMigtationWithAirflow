...
# ==========================================================
# LARGE-3: MySQL -> MySQL 대용량 단순 적재 (테이블 그룹 3)
# ==========================================================
...
TABLE_CONFIG: Dict[str, Dict[str, Any]] = {
    "big_act_log": {
        "range": {"col": "log_id", "start": 1, "end": 300_000_000},  # 약 3억
    },
    "big_audit_trail": {
        "range": {"col": "audit_id", "start": 1, "end": 150_000_000},  # 약 1.5억
    },
    "big_msg_queue": {
        "range": {"col": "msg_id", "start": 1, "end": 40_000_000},  # 약 4천만
    },
}
...
def _mysql_copy_table(table: str, **context: Any) -> None:
    ...
    try:
        ...
        for r_start, r_end in id_ranges:
            ...
            print(f"[LARGE-3][{table}] range {r_start}~{r_end} {query}", flush=True)
            ...
            try:
                ...
                pass
            print(
                f" >>> [LARGE-3][{table}] range {r_start}~{r_end} rows={chunk_in_range} total={total}",
                flush=True,
            )

        print(f"[LARGE-3][{table}] finished total={total}", flush=True)
    except Exception as e:
        print(f"!!! [LARGE-3][{table}] failed: {e}", flush=True)
        raise
    finally:
        if src_conn:
            try:
