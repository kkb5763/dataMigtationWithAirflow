...
# ==========================================================
# LARGE-2: MySQL -> MySQL 대용량 단순 적재 (테이블 그룹 2)
# ==========================================================
# LARGE-1 과 동일 패턴. TABLE_CONFIG·접속 정보만 이 파일에서 관리.
...
TABLE_CONFIG: Dict[str, Dict[str, Any]] = {
    "big_ord_mst": {
        "range": {"col": "ord_no", "start": 1, "end": 120_000_000},  # 약 1.2억
    },
    "big_ord_dtl": {
        "range": {"col": "dtl_no", "start": 1, "end": 200_000_000},  # 약 2억
    },
    "big_ord_pay": {
        "range": {"col": "pay_id", "start": 1, "end": 60_000_000},  # 약 6천만
    },
}
...
def _mysql_copy_table(table: str, **context: Any) -> None:
    ...
    try:
        ...
        for r_start, r_end in id_ranges:
            ...
            print(f"[LARGE-2][{table}] range {r_start}~{r_end} {query}", flush=True)
            ...
            try:
                ...
                pass
            print(
                f" >>> [LARGE-2][{table}] range {r_start}~{r_end} rows={chunk_in_range} total={total}",
                flush=True,
            )

        print(f"[LARGE-2][{table}] finished total={total}", flush=True)
    except Exception as e:
        print(f"!!! [LARGE-2][{table}] failed: {e}", flush=True)
        raise
    finally:
        if src_conn:
            try:
