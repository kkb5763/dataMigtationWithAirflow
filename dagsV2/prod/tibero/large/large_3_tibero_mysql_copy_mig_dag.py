...
# LARGE-3: Tibero6 -> MySQL 대용량 (테이블 그룹 3) — sample chunk 패턴
...
TABLE_CONFIG: Dict[str, Tuple[str, int, int]] = {
    "LOG.BIG_ACT_LOG": ("LOG_ID", 1, 300_000_000),
    "LOG.BIG_AUDIT_TRAIL": ("AUDIT_ID", 1, 150_000_000),
    "LOG.BIG_MSG_QUEUE": ("MSG_ID", 1, 40_000_000),
}
...
def _worker(table: str, range_col: str, start: int, end: int, **context: Any) -> None:
    ...
    try:
        ...
        while current <= end:
            ...
            print(f"[LARGE-TIB-3][{table}] 실행 쿼리: {query}", flush=True)
            t_cur.execute(query)
            ...
            while True:
                ...
                total += len(rows)
            print(
                f"[LARGE-TIB-3][{table}] range {current}~{current_end} done "
...
with DAG(
    dag_id="large_tibero_mysql_copy_3_v1",
    ...
    for table_name, (col, start, end) in TABLE_CONFIG.items():
        PythonOperator(
            ...
            pool="large_tibero_mysql_copy_3",
            pool_slots=1,
