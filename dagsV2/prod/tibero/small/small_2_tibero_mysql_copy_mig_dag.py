...
# SMALL-2: Tibero6 -> MySQL (테이블 그룹 2) — sample_tibero_chunk_mig_dag.py 패턴
...
TABLE_CONFIG: Dict[str, Tuple[str, int, int]] = {
    "ORDER.ORD_HDR": ("ORD_NO", 1, 80_000),
    "ORDER.ORD_LINE": ("LINE_NO", 1, 120_000),
    "ORDER.ORD_STAT": ("STAT_ID", 1, 40_000),
}
...
def _worker(table: str, range_col: str, start: int, end: int, **context: Any) -> None:
    ...
    try:
        ...
        while current <= end:
            ...
            print(f"[SMALL-TIB-2][{table}] 실행 쿼리: {query}", flush=True)
            t_cur.execute(query)
            ...
            while True:
                ...
                total += len(rows)
            print(
                f"[SMALL-TIB-2][{table}] range {current}~{current_end} done "
...
with DAG(
    dag_id="small_tibero_mysql_copy_2_v1",
    ...
    for table_name, (col, start, end) in TABLE_CONFIG.items():
        PythonOperator(
            ...
            pool="small_tibero_mysql_copy_2",
            pool_slots=1,
