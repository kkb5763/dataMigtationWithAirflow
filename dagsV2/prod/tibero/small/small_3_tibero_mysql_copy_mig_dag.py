...
# SMALL-3: Tibero6 -> MySQL (테이블 그룹 3) — sample_tibero_chunk_mig_dag.py 패턴
...
TABLE_CONFIG: Dict[str, Tuple[str, int, int]] = {
    "LOG.ACT_LOG": ("LOG_ID", 1, 100_000),
    "LOG.ERR_LOG": ("ERR_ID", 1, 60_000),
    "LOG.SYNC_LOG": ("SYNC_ID", 1, 40_000),
}
...
def _worker(table: str, range_col: str, start: int, end: int, **context: Any) -> None:
    ...
    try:
        ...
        while current <= end:
            ...
            print(f"[SMALL-TIB-3][{table}] 실행 쿼리: {query}", flush=True)
            t_cur.execute(query)
            ...
            while True:
                ...
                total += len(rows)
            print(
                f"[SMALL-TIB-3][{table}] range {current}~{current_end} done "
...
with DAG(
    dag_id="small_tibero_mysql_copy_3_v1",
    ...
    for table_name, (col, start, end) in TABLE_CONFIG.items():
        PythonOperator(
            ...
            pool="small_tibero_mysql_copy_3",
            pool_slots=1,
