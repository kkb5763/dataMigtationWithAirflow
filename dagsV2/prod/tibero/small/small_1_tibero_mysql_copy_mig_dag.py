...
# ==========================================================
# SMALL-1: Tibero6 -> MySQL (테이블 그룹 1)
# ==========================================================
# sample_tibero_chunk_mig_dag.py 와 동일: STEP_SIZE 범위 단위 SELECT 반복
# 소량이어도 Tibero 는 구간 쿼리 패턴 사용 (end 값만 작게 설정)
# Pools: small_tibero_mysql_copy_1 / slots 4
...
# table: (range_col, start, end) — SCHEMA.TABLE
TABLE_CONFIG: Dict[str, Tuple[str, int, int]] = {
    "MEMBER.MBR_CFG": ("CFG_ID", 1, 50_000),
    "MEMBER.MBR_CODE": ("CODE_ID", 1, 30_000),
    "MEMBER.MBR_EXT": ("EXT_ID", 1, 20_000),
}
    ...
    **context: Any,
    ...
    try:
        ...
        while current <= end:
            ...
            print(f"[SMALL-TIB-1][{table}] 실행 쿼리: {query}", flush=True)
            t_cur.execute(query)
            ...
            print(
                f"[SMALL-TIB-1][{table}] range {current}~{current_end} done "
...
with DAG(
    ...
    for table_name, (col, start, end) in TABLE_CONFIG.items():
        PythonOperator(
            ...
            pool="small_tibero_mysql_copy_1",
            pool_slots=1,
            op_kwargs={
                "table": table_name,
                "range_col": col,
