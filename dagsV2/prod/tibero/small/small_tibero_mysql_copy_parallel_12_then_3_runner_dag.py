...
# SMALL Tibero (1·2 병렬) → 3 순차 오케스트레이터
# Pool: small_tibero_mysql_runner / slots 2
...
with DAG(
    dag_id="small_tibero_mysql_copy_parallel_12_then_3_runner_v1",
    default_args={"owner": "small", "retries": 0},
    start_date=datetime(2026, 4, 1),
    schedule_interval=None,
    catchup=False,
    tags=["small", "tibero", "orchestrator", "parallel", "sequential"],
    max_active_runs=1,
) as dag:
    run_1 = _trigger(
        "run__small_tibero_mysql_copy_1_v1",
        SMALL_1_DAG_ID,
        pool="small_tibero_mysql_runner",
    )
    run_2 = _trigger(
        "run__small_tibero_mysql_copy_2_v1",
        SMALL_2_DAG_ID,
        pool="small_tibero_mysql_runner",
    )
