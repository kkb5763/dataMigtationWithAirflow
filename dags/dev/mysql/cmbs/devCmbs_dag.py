...
TABLE_CONFIG: Dict[str, Tuple[str, Optional[Union[int, str]], Optional[Union[int, str]]]] = {
    "user_master": ("user_id", 1, 100000),
    "order_history": ("order_dt", "2026-01-01", None),  # end_val=None => 전체(하한만 적용)
    "system_log": ("reg_dt", "{{ ds }}", "{{ next_ds }}"),
    "code_definition": ("code", None, None),  # start/end 모두 None => 전체 조회
}
...
def _mysql_to_mysql_etl(
    ...
    **context: Any,
    ...
    try:
        ...
        # Source는 대용량 조회를 위해 SScursor(Server-side cursor) 사용
